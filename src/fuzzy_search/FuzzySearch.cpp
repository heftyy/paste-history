...
namespace FuzzySearch
{
	...
	int CalculateScore(const std::string& pattern, const std::string& str, int pattern_index, int str_index, 
					   int filename_start_index, MatchMode match_mode, const std::vector<int>& matches, int matches_length)
	{
	...
	int FuzzyMatch(const std::string& pattern, const std::string& str, int filename_start_index, MatchMode match_mode, std::vector<int>& out_matches)
	{
		...
		for (int pattern_index = 0; pattern_index < pattern.length(); ++pattern_index)
		{
			...
			// When pattern contains a space, start a search from the beginning of str again
			...
			for (int str_index = str_start; str_index < str.length(); ++str_index)
			{
				...
				{
					int match_score = CalculateScore(pattern, str, pattern_index, str_index, filename_start_index, match_mode, matches, search_pattern_index - pattern_index);
